...
function BANNER() {
  ...
  if (windowidth < 1024) {
    ...
    var mobbackground = document.querySelectorAll(
      ".BannerItem .BannerItemBackgroundMobile"
    );
    for (var idx = 0; idx < mobbackground.length; idx++) {
      mobbackground[idx].style.display = "";
    }
    var pcbackground = document.querySelectorAll(
      ".BannerItem .BannerItemBackgroundPC"
    );
    for (var idx = 0; idx < pcbackground.length; idx++) {
      pcbackground[idx].style.display = "";
    }
    var mobileImg = document.querySelectorAll(
      ".BannerItem .BannerItemImageMobile"
    );
    ...
    var BannerCurrentPosion = -(
      (windowidth - 40) * 2 +
      carouselindex * (windowidth - 40)
    );

    $BannerStage.style.transform =
      "translateX(" + BannerCurrentPosion.toString() + "px)";
      ...
      $BannerStage.addEventListener("mousemove", function (e) {
        if (windowidth < 1024) {
          if (pressed) {
            ...
            $BannerStage.style.transform =
              "translateX(" +
              (BannerCurrentPosion - (startX - currentX)).toString() +
              "px)";
          }
        }
      });
